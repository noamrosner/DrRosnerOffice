...
def ramatHahayalMessage(date, day, time, name, exam):
    msg = f"""
    שלום {name}, 
     זו תזכורת מהמרפאה של ד״ר רוזנר לבדיקת
     {exam} שנקבעה לך ליום {day} {date} בשעה {time}
    באסותא רמת החייל, רחוב הברזל 12 תל אביב קומה 5 מכון גסטרו. 

    *נא להגיע חצי שעה טרם שעת הבדיקה.* 
    באם אין בכוונתך להגיע נא להודיע למספר זה בהודעה חוזרת. 
...
def ramatHahyalSendToList():
    ...
    for i in range(2, sheet.max_row):
        try:
            msg = ramatHahayalMessage(sheet[f"A{i}"].value.strftime("%m/%d/%Y"), sheet[f"B{i}"].value,
...
def rannanaSendToList():
    ...
    for i in range(2, sheet.max_row):
        try:
            msg = rannanaMessage(sheet[f"A{i}"].value.strftime("%m/%d/%Y"), sheet[f"B{i}"].value,
                                  sheet[f"C{i}"].value.strftime("%H:%M"), sheet[f"D{i}"].value, sheet[f"G{i}"].value)
            validPhone(f'{sheet[f"F{i}"].value}')
            sendMessage(validPhone(sheet[f"F{i}"].value), msg)
            sheet[f"I{i}"].value = "Sent successfully"
